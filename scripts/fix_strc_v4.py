...
def fix_citations_final_v4(content):
    ...
    if works_cited_start != -1:
        ...
        def citation_subber(match):
            ...
            # Group 1 is prefix, Group 2 is num
        ...
        # Regex: (optional prefix) followed by num. 
        # We look for .N (preceded by word) or [N] (preceded by space)
        # 1. .N where N is 1-2 digits and is in dict
        text_fixed = re.sub(r'(?<=\w)\.(\d{1,2})(?!\d)', lambda m: citation_subber(type('M', (object,), {'group': lambda _, i: '' if i==1 else m.group(1)})()), text_before)
        # 2. \s[N] where N is 1-2 digits
        text_fixed = re.sub(r'(\s)\[(\d{1,2})\]', lambda m: citation_subber(type('M', (object,), {'group': lambda _, i: m.group(1) if i==1 else m.group(2)})()), text_fixed)
...
fixed = fix_citations_final_v4(original)
